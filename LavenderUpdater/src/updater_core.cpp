...
static void freeze_other_threads(std::vector<HANDLE> &out)
{
...
static void unfreeze_threads(std::vector<HANDLE> &handles)
{
...
extern "C" __declspec(dllexport) void RunUpdater()
{
    ...
    // Freeze and prompt
    log_message("RunUpdater: freezing process for update prompt");
    std::vector<HANDLE> frozenThreads;
    freeze_other_threads(frozenThreads);
    ...
    {
        // Yes
        unfreeze_threads(frozenThreads);
